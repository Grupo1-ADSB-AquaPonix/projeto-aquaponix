...
function buscarUltimasMedidas(idLocal, limite_linhas, idSensor) {

    var instrucaoSql = `SELECT idDado,
	valor, 
    DATE_FORMAT(dtColeta,'%H:%i:%s') AS 'data_coleta'
    FROM dadoscapturados
    WHERE fkSensor = ${idSensor} AND fkLocal = ${idLocal}
...
function buscarMedidasEmTempoReal(idLocal, idSensor) {
